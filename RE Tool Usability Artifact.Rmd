---
title: "RE Taxonomy Analysis / Visualizations"
...
## The next several code blocks are used for generating some of the visualizations included in the paper (commented out to save space) as well as
## melting the data frame to aggregate the counts needed for the Chi Squared tests performed. The dataframes aggregating Analysis phase results (G1)
## are saved as "bound." The dataframe for plugin types are saved as "castframe". The dataframe for Functionality type are saved as "Funcframe"

## The Chi Squared tests compare input, output content, and output method against the other variables of our data set. To properly run the Chi Squared
## tests the same covariate block needs to be executed across all variables. For example, if you wanted to run the Input Chi Squared tests the
## following code blocks should be executed first: "Input vs G1", "Input vs Stat/Dyn", and "Input vs Framework". Afterwards the three newly created
## dataframes need to be further aggregated in the code blocks starting with the code block labeled "Group inputs together".


##*********** START OF CODE BLOCKS FOR ANALYSIS PHASE *********** 
...
## Code to produce the bars used to create Figure 2 in Section 5 of the paper. This code block must be run after the one above
## labeled "Input vs G1" to produce the right results.
```{r}
...
## Code to produce the bars used to create Figure 3 in Section 5 of the paper. This code block must be run after the one above
## labeled "OUTPUT CONTENT vs G1" to produce the right results.
```{r}
bound$G1 = ordered(bound$G1, levels = c("Experimentation", "Subcomponent", "Overview"))
...
## Code to produce the bars used to create Figure 4 in Section 5 of the paper. This code block must be run after the one above
## labeled "Output Method vs G1" to produce the right results.
```{r}
bound$G1 = ordered(bound$G1, levels = c("Experimentation", "Subcomponent", "Overview"))
...
##*********** START OF CODE BLOCKS FOR FUNCTIONALITY TYPE *********** 
...
##*********** START OF CODE BLOCKS FOR PLUGIN TYPE *********** 
...
## The next three codeblocks are used to aggregate the data as previously mentioned. To continue the previous example of performing the Chi Squared
## tests for input type, the codeblock labeled "Group inputs together" must be run three separate times with slight modifications. Each of the three
## dataframes created in the previous blocks (castframe, Funcframe, G1frame) should be set to castdf. There are commented out lines to make it simpler.
## The line at the end of the block should also be set to overwrite the dataframe with the newly aggregated results.

##Group inputs together
```{r}
#Set df you want to group together to castdf
#castdf = Funcframe
#castdf = G1frame
castdf = castframe
...
#Save resulting df for chisq comps later
castframe = castdf
#Funcframe = castdf
#G1frame = castdf
```
...
##GROUP OUTC TOGETHER
```{r}
#Set df you want to group to castdf
castdf = Funcframe
#castdf = G1frame
#castdf = castframe
...
## Bellow are the blocks for the Chi squared tests. After performing the previous steps of generating the data frame and further grouping them,
## run the requisit code block for the results.
...
```{r}
#Code to save the state of Output content to restore without painstaking process
OCg1frame = G1frame
OCFuncframe = Funcframe
OCcastframe = castframe

#restore so the code can work easier!
G1frame= OCg1frame
Funcframe = OCFuncframe
castframe = OCcastframe
